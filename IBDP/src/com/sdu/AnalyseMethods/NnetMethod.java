...
public class NnetMethod extends BasicMethod{
	...
	public DataFile BasedOnR(int index, DataFile dataFile, Admin user, Project project, JSONObject projectJSON, JSONArray algorithmJSON)
	{
		...
		String size=params.getJSONObject(2).getString("value");
		String maxit=params.getJSONObject(3).getString("value");
 	...
 	if(type.equals("回归"))
 	{
 		c.eval("model <- nnet("+formula+", data=datafile,size="+size+",decay=5e-4,maxit="+maxit+",linout=TRUE)");
	...
	c.eval("model<-ml_multilayer_perceptron(data_tbl, "+formula+", iter.max= "+maxit+", layers=c("+size+")");
