...
public class KNNMethod extends BasicMethod{
	public DataFile beiginAnalyse(int index, DataFile dataFile, Admin user, Project project, JSONObject projectJSON, JSONArray algorithmJSON)
	{
		...
		String  k=params.getJSONObject(2).getString("value");
    	...
    	System.out.println("输入参数为："+filepath+ "  "+dataFileName+"  "+y+"~"+x+"  "+k);
	...
	c.eval("datafile[,\""+y+"\"]=as.factor(datafile[,\""+y+"\"])");
 	c.eval("kknn.model <- kknn("+y+"~"+x+", train=datafile,test=datafile,k="+k+")");
	
 	String resultFileName=dataFileName.substring(0,dataFileName.lastIndexOf('.'))+"_KNN";
 	...
 	//假如文件是结果文件的话，文件类别为ResultFile，存储为txt数据或者是图片
 	 	if(index==algorithmJSON.length()-1)
		{
			...
			c.eval("print(pairs("+y+"~"+x+",data=datafile))");
			c.eval("print(summary(kknn.model))");
			c.eval("dev.off()");
			
	 		c.eval("sink(\""+resultFileName+".txt\")");
			c.eval("print(predict(kknn.model))");
			c.eval("sink()");
			//生成结果文件并进行保存修改数据库
			//保存图片
