					...
					<ul class="nav ace-nav">
						...
						<li class="light-blue dropdown-modal">
							<a data-toggle="dropdown" href="#" class="dropdown-toggle">
								<img id="userPhoto" class="nav-user-photo" src="" alt="用户头像" />
								<span class="user-info">
									<small>Welcome,</small>
									<%
									Admin user=(Admin)session.getAttribute("user");
									int flag = user.getIsAdmin();
									if(user!=null)
									{		
									%>
									 <span id="admin_username"></span>
<!-- 									<%=user.getName() %> -->
									<%}%>
								</span>
							...
							<ul id="userManage" class="user-menu dropdown-menu-right dropdown-menu dropdown-yellow dropdown-caret dropdown-close">
<!-- 								<li id="personCenter"> -->
<!-- 									<a href="#"> -->
...
<script type="text/javascript">
	function userManagerLi(){
		if(<%=flag%>==0){
			$("#personCenter").after("<li id='userLi'><a href='user.jsp'><i class='ace-icon fa fa-users'></i> 用户管理</a></li>");
		}
	} 
	$(function(){
			$.post("user_showUser.action").success( function(response) {
				  console.log("开始执行");
				  var admin = JSON.parse(response.DataJson);
                    $("#userPhoto").attr("src",admin.imgName);
              		$("#admin_username").text(admin.name);
              		admin_imgName = admin.imgName;
			...
			if(data.count>0){
				...
				for( i = 0;i<data.count&& i<5;i++){
					if( $("#"+message[i].a_id).length<=0){
						$("#dropdown_menu").append("<li onclick='clickOn(this)' id='"+message[i].a_id+"'><a href='datafile.jsp'><div class='clearfix'><span class='pull-left'><i class='btn btn-xs no-hover btn-pink fa fa-comment'></i>"+message[i].a_content+"</span></div></a><li>");
					}
				}
